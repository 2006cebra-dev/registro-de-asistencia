<x-app-layout>
    ...
    <x-slot name="header">
        <div>
            <span class="text-white text-lg font-bold tracking-tight">Mi Perfil</span>
            <p class="text-sm text-gray-400">Gestiona tu información personal</p>
        </div>
    </x-slot>

    <div class="max-w-2xl mx-auto space-y-5">
        <div class="relative overflow-hidden rounded-xl bg-[#132347] border border-white/10 p-6 sm:p-8 shadow-lg">
            ...
            <div class="relative">
                <div class="flex flex-col items-center mb-6 pb-6 border-b border-white/10">
                    <div class="relative">
                        @if ($foto)
                            <img id="preview-foto" src="{{ $foto }}" alt="{{ Auth::user()->name }}" class="w-24 h-24 rounded-2xl object-cover border-2 border-yellow-500/30 shadow-xl ring-4 ring-yellow-500/10">
                        @else
                            <div id="preview-foto" class="w-24 h-24 rounded-2xl bg-gradient-to-br from-yellow-500 to-yellow-700 flex items-center justify-center shadow-xl border-2 border-yellow-500/30 ring-4 ring-yellow-500/10">
                    ...
                    <span class="text-sm px-3 py-0.5 rounded-full mt-1 font-medium
                        {{ Auth::user()->rol === 'docente' ? 'bg-yellow-500/20 text-yellow-400 ring-1 ring-yellow-500/20' : (Auth::user()->rol === 'supervisor' ? 'bg-purple-500/20 text-purple-400 ring-1 ring-purple-500/20' : 'bg-blue-500/20 text-blue-400 ring-1 ring-blue-500/20') }}">
                ...
                <form method="post" action="{{ route('profile.update') }}" enctype="multipart/form-data">
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-4">
                                <label for="foto" class="cursor-pointer inline-flex items-center gap-2 px-4 py-2.5 bg-white/10 text-gray-300 rounded-lg text-sm font-medium hover:bg-white/20 hover:text-white transition-all border border-white/20 hover:border-yellow-500/30">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                    Elegir foto
                                </label>
                                <span id="foto-name" class="text-sm text-gray-500">Ningún archivo seleccionado</span>
                            </div>
                    ...
                    <div class="flex items-center gap-4 pt-6 mt-6 border-t border-white/10">
                        <button type="submit" class="inline-flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-yellow-600 to-yellow-800 text-white rounded-xl font-semibold text-sm shadow-lg hover:shadow-xl hover:from-yellow-500 hover:to-yellow-700 hover:scale-105 active:scale-95 transition-all duration-200">
        ...
        <div class="rounded-xl bg-[#132347] border border-white/10 p-5 sm:p-8 shadow-lg">
            <div class="flex items-center gap-3 mb-6 pb-4 border-b border-white/10">
                <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-red-600 to-red-800 flex items-center justify-center">
                ...
                <div>
                    <h3 class="text-white font-semibold">Cerrar Sesión</h3>
                    <p class="text-gray-400 text-xs">Finalizar tu sesión actual</p>
                </div>
            </div>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="w-full inline-flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-red-600 to-red-800 text-white rounded-xl font-bold text-sm shadow-lg hover:from-red-500 hover:to-red-700 hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 cursor-pointer">
        ...
        function previewFile(input) {
            ...
            reader.onload = function(e) {
                ...
                if (preview.tagName === 'IMG') {
                    ...
                    img.id = 'preview-foto';
                    img.className = 'w-24 h-24 rounded-2xl object-cover border-2 border-yellow-500/30 shadow-xl ring-4 ring-yellow-500/10';

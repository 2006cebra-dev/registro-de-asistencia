<x-app-layout>
    <x-slot name="header">
        <div>
            <span class="text-white text-lg font-bold tracking-tight">Estudiantes</span>
            <p class="text-sm text-gray-400">Gestiona los estudiantes del sistema</p>
        </div>
    </x-slot>

    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        <div class="card-3d rounded-xl bg-[#132347] border border-white/10 p-4 text-center">
        ...
        <div class="card-3d rounded-xl bg-[#132347] border border-red-500/20 p-4 text-center">
        ...
        <div class="sm:hidden space-y-3 p-4">
            ...
            <div class="p-4 rounded-lg bg-white/[0.03] border border-white/[0.06] hover:border-yellow-500/20 transition-all">
                <div class="flex items-center gap-3 mb-3">
                    @if ($fotoEst)
                        <img src="{{ $fotoEst }}" alt="" class="w-10 h-10 rounded-lg object-cover border border-yellow-500/20 shrink-0">
                    @else
                        <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-green-600 to-green-800 flex items-center justify-center shrink-0">
                            <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                        </div>
                    @endif
                    <div class="min-w-0 flex-1">
                        <p class="text-sm font-medium text-white truncate">{{ $estudiante->nombre }}</p>
                        <p class="text-xs text-gray-500 truncate">{{ $estudiante->email }}</p>
                    </div>
                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium shrink-0 {{ $estudiante->activo ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300' }}">{{ $estudiante->activo ? 'Activo' : 'Inactivo' }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-500/20 text-yellow-300">{{ $estudiante->curso?->nombre ?? 'Sin curso' }}</span>
                    <div class="flex items-center gap-1">
                        <a href="{{ route('estudiantes.show', $estudiante) }}" class="p-2 rounded-lg text-blue-400 hover:text-blue-300 hover:bg-blue-500/10 transition-colors"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg></a>
                        <a href="{{ route('estudiantes.edit', $estudiante) }}" class="p-2 rounded-lg text-yellow-400 hover:text-yellow-300 hover:bg-yellow-500/10 transition-colors"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></a>
                        <form action="{{ route('estudiantes.toggle', $estudiante) }}" method="POST" class="inline">
                            @csrf @method('PATCH')
                            <button type="submit" class="p-2 rounded-lg cursor-pointer {{ $estudiante->activo ? 'text-red-400 hover:text-red-300 hover:bg-red-500/10' : 'text-green-400 hover:text-green-300 hover:bg-green-500/10' }} transition-colors">
                                @if ($estudiante->activo)
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                @else
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            ...
            <div class="text-center py-10">
                <svg class="w-12 h-12 text-white/20 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                <p class="text-white/40 text-sm">No hay estudiantes registrados</p>
            </div>
            @endforelse
        </div>
